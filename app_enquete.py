...
# ═══════════════════════════════════════════════════════════════
# AUTHENTIFICATION & ROUTAGE MULTI-RÔLES
# ═══════════════════════════════════════════════════════════════
...
# Base de données d'utilisateurs simulée (Hardcodée pour la démo)
USERS = {
    "admin": {"password": "admin123", "role": "admin"},
    "enqueteur": {"password": "pass123", "role": "user"}
}

if not st.session_state.logged_in:
    st.markdown("<div style='max-width: 400px; margin: 100px auto; padding: 2.5rem; background: var(--bg-card); border-radius: var(--radius-lg); box-shadow: var(--shadow-lg); border: 1px solid var(--border);'>", unsafe_allow_html=True)
    st.markdown("<h2 style='text-align: center; color: var(--primary-dark); margin-bottom: 2rem; font-family: Outfit;'>🔐 Accès Restreint</h2>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown(f"🔑 **Rôle :** `{'Administrateur' if st.session_state.role == 'admin' else 'Enquêteur'}`")
    st.divider()
...
# ═══════════════════════════════════════════════════════════════
# VUE ADMINISTRATEUR (DASHBOARD)
# ═══════════════════════════════════════════════════════════════
if st.session_state.role == "admin":
    st.markdown("<h1 style='color: var(--primary-dark); font-family: Outfit;'>📊 Tableau de Bord Administratif</h1>", unsafe_allow_html=True)
    st.markdown("Bienvenue dans l'espace d'analyse des données collectées sur le terrain.")
    
    with st.spinner("Récupération des données depuis Google Sheets..."):
        gc, sh = get_gspread_client()
        if sh is not None:
            try:
                onglet_global = sh.worksheet("Global")
                data_global = onglet_global.get_all_values()
                if data_global and len(data_global) > 0:
                    colonnes_brutes = data_global[0]
                    
                    if len(colonnes_brutes) > 0 and str(colonnes_brutes[0]).strip().lower() == "code fiche":
                        # La première ligne est bien un en-tête
                        lignes = data_global[1:] if len(data_global) > 1 else []
                        
                        # Déduplication des noms de colonnes (PyArrow plante si valeurs dupliquées)
                        colonnes_nettoyees = []
                        vus = set()
                        for c in colonnes_brutes:
                            c_str = str(c).strip()
                            if c_str == "": c_str = "Vide"
                            if c_str in vus:
                                suffix = 1
                                while f"{c_str}_{suffix}" in vus: suffix += 1
                                c_str = f"{c_str}_{suffix}"
                            vus.add(c_str)
                            colonnes_nettoyees.append(c_str)
                            
                        df_global = pd.DataFrame(lignes, columns=colonnes_nettoyees)
                    else:
                        # La première ligne contient des données brutes, l'en-tête a probablement été effacé
                        lignes = data_global
                        df_global = pd.DataFrame(lignes, columns=[f"Colonne {i+1}" for i in range(len(colonnes_brutes))])
                    
                    # KPIs
                    st.markdown("<br><h3>📈 Indicateurs Clés</h3>", unsafe_allow_html=True)
                    col1, col2, col3 = st.columns(3)
                    
                    with col1:
                        st.markdown(f'<div class="info-card"><div class="info-card-title">📝 Total Enquêtes</div><h2 style="margin:0;">{len(df_global)}</h2></div>', unsafe_allow_html=True)
                    
                    with col2:
                        if "Cultures enquêtées" in df_global.columns:
                            val = df_global["Cultures enquêtées"].nunique()
                            st.markdown(f'<div class="info-card"><div class="info-card-title">🌱 Cultures Uniques</div><h2 style="margin:0;">{val}</h2></div>', unsafe_allow_html=True)
                        else:
                            st.markdown('<div class="info-card"><div class="info-card-title">🌱 Cultures Uniques</div><h2 style="margin:0;">N/A</h2></div>', unsafe_allow_html=True)
                            
                    with col3:
                        if "Zone" in df_global.columns:
                            val = df_global["Zone"].nunique()
                            st.markdown(f'<div class="info-card"><div class="info-card-title">🗺️ Zones Couvertes</div><h2 style="margin:0;">{val}</h2></div>', unsafe_allow_html=True)
                        else:
                            st.markdown('<div class="info-card"><div class="info-card-title">🗺️ Zones Couvertes</div><h2 style="margin:0;">N/A</h2></div>', unsafe_allow_html=True)
                            
                    st.markdown("<br>### 📋 Données Globales Collectées", unsafe_allow_html=True)
                    st.dataframe(df_global, use_container_width=True)
                else:
                    st.info("Aucune donnée collectée pour le moment.")
            except gspread.exceptions.WorksheetNotFound:
                st.info("L'onglet 'Global' n'existe pas encore. Aucune donnée n'a été soumise.")
            except Exception as e:
                st.error(f"Erreur lors de la lecture des données : {e}")
                
    # On arrête l'exécution ici pour l'Admin, afin qu'il ne voie pas le formulaire utilisateur
    st.stop()
