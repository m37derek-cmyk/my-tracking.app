...
def calculate_score(row):
    ...
    # التعديل هنا: قيام الليل والقرآن صاروا بنعم/لا
    if row.get('قيام') == 'نعم': score += 8
    if row.get('القرآن') == 'نعم': score += 8
...
with tab1:
    ...
    with st.form("entry_form"):
        ...
        st.markdown("<br>", unsafe_allow_html=True)
        # --- التعديلات: صناديق اختيار للقيام والقرآن ---
        c_q1, c_q2 = st.columns(2)
        qiyam = c_q1.checkbox("🌙 قيام الليل (+8)")
        quran = c_q2.checkbox("📖 الورد القرآني (+8)")
        # -----------------------------------------------
        ...
        if submit:
            ...
            if not user_specific_df.empty and day_date in user_specific_df['التاريخ'].astype(str).values:
                ...
                row = [
                    ...
                    "نعم" if qiyam else "لا", "نعم" if quran else "لا", # تعديل الحفظ
