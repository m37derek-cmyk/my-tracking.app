...
def calculate_score(row):
    ...
    score += quran_map.get(quran_val, 0)
    
    # 4. قيام الليل (نظام التدرج بالنقاط)
    ...
    score += qiyam_map.get(qiyam_val, 0)
...
with tab1:
    ...
    with st.form("entry_form"):
        ...
        # الروحانيات (تعديل: استخدام Slider للتدرج كما طلبت)
        with st.expander("📖 الروحانيات (القرآن والقيام)", expanded=False):
            ...
            with col_z2:
                st.markdown("**🌙 التدرج في العبادة**")
                # ⚠️ شريط التدرج (Select Slider) هو الأنسب لكلمة "التدرج"
                inputs['qiyam'] = st.select_slider("قيام الليل (كم ركعة؟)", options=["0", "ركعتان", "4 ركعات", "6 ركعات", "8 ركعات"])
                inputs['quran'] = st.select_slider("الورد القرآني (الكمية؟)", options=["0", "ثمن", "ربع", "نصف", "حزب", "حزبين"])
        ...
        if submit:
            ...
            if is_duplicate:
                ...
                row = [
                    ...
                    inputs['qiyam'], # قيمة التدرج
                    inputs['quran'], # قيمة التدرج
